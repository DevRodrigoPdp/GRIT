...
public interface PlanNutricionRepository extends JpaRepository<PlanNutricion, UUID> {
    ...
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("UPDATE PlanNutricion p SET p.activo = false WHERE p.entrenador.id = :entrenadorId AND p.activo = true")
    void desactivarPlanesPorEntrenador(@Param("entrenadorId") UUID entrenadorId);
