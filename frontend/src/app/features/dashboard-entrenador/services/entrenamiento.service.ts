...
export class EntrenamientoService {
  ...
  constructor() {
    ...
    this.busqueda$
      .pipe(
        ...
        switchMap((q) => {
          if (q.length < 2) {
            this.sugerenciasCache.set([]);
            return of(null);
          }
          this.cargandoSugerencias.set(true);
          return this.http
            .get<
              PagedResponse<EjercicioSugerencia>
            >(`${this.API_EJ}/search`, { params: { q, page: 0, size: 15 }, withCredentials: true })
  ...
  getRutinas(atletaId: string): Observable<Rutina[]> {
    return this.http.get<ApiResponse<any[]>>(`${this.API}/rutinas`, { params: { atletaId } }).pipe(
      map((r) =>
        r.data.map((x: any) => ({
          id: x.id,
          atletaId: x.atletaId,
